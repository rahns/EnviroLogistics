...
* @returns {any}
*/
export function get_routing_locations(problem) {
    try {
        var ret = wasm.get_routing_locations(addBorrowedObject(problem));
...
* @returns {any}
*/
export function validate_pragmatic(problem, matrices) {
    try {
        var ret = wasm.validate_pragmatic(addBorrowedObject(problem), addBorrowedObject(matrices));
        return takeObject(ret);
    } finally {
        heap[stack_pointer++] = undefined;
        heap[stack_pointer++] = undefined;
...
* @returns {any}
*/
export function convert_to_pragmatic(format, inputs) {
    try {
        var ptr0 = passStringToWasm0(format, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        var len0 = WASM_VECTOR_LEN;
        var ret = wasm.convert_to_pragmatic(ptr0, len0, addBorrowedObject(inputs));
        return takeObject(ret);
    } finally {
        heap[stack_pointer++] = undefined;
    }
...
* @returns {any}
*/
export function solve_pragmatic(problem, matrices, config) {
    try {
        var ret = wasm.solve_pragmatic(addBorrowedObject(problem), addBorrowedObject(matrices), addBorrowedObject(config));
        return takeObject(ret);
    } finally {
        heap[stack_pointer++] = undefined;
        heap[stack_pointer++] = undefined;
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL('vrp_cli_bg.wasm', import.meta.url);
    }
    const imports = {};
    imports.wbg = {};
